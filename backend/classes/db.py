...
class DBManager:
    ...
    def get_product_info(self, product_id):
        self.c.execute(
            "SELECT product_name, category_id FROM products WHERE product_id = ?",
            (product_id,),
        )
        return self.c.fetchone()
    ...
    def add_purchase(self, store_id, items):
        ...
        for item in items:
            self.c.execute(
                """INSERT INTO order_details(order_id, product_id, qty, in_stock)
            VALUES (?,?,?,?)""",
                (
                    order_id,
                    item[0].product_id,
                    item[1],
                    item[0].in_stock(store_id, item[1]),
                ),
            )
        self.conn.commit()
        return order_id
    ...
    def retrieve_order(self, order_id):
        self.c.execute(
            """SELECT store_id, status_id FROM orders WHERE order_id = ?""", (order_id,)
        )
        store_id, status_id = self.c.fetchone()
        # add retrieve status
        self.c.execute(
            """SELECT product_id, qty, shipped FROM order_details WHERE order_id = ?""",
            (order_id,),
        )
    ...
    def ship_item(self, order_id, store_id, product_id, qty):
        try:
            ...
            self.c.execute(
                """UPDATE stock SET qty = qty - ? WHERE product_id = ? AND store_id = ?""",
        ...
        self.c.execute(
            """UPDATE order_details SET shipped = 1 WHERE order_id = ? AND product_id = ?""",
            (order_id, product_id),
        )
        self.conn.commit()
    ...
    def update_order_status(self, order_id):
        ...
        self.c.execute(
            """SELECT shipped FROM order_details WHERE order_id = ?""", (order_id,)
        )
        shipped = self.c.fetchall()
        if all(shipped):
            self.c.execute(
                """UPDATE orders SET status_id = 2 WHERE order_id = ?""", (order_id,)
            )
            self.conn.commit()
        # if some items are shipped, update order status to order_id=1
        elif any(shipped):
            self.c.execute(
                """UPDATE orders SET status_id = 1 WHERE order_id = ?""", (order_id,)
            )
            self.conn.commit()
        ...
        # if no items are shipped, update order status to order_id=0
            ...
            self.c.execute(
                """UPDATE orders SET status_id = 0 WHERE order_id = ?""", (order_id,)
            )
            self.conn.commit()
    ...
    def add_restock(self, store_id, items):
        ...
        for item in items:
            self.c.execute(
                """INSERT INTO restock_order_details(restock_order_id, product_id, qty)
            VALUES (?,?,?)""",
                (restock_id, item[0].product_id, item[1]),
            )
        return restock_id
    ...
    def receive_restock(self, restock_id, store_id, product_id, qty):
        # update stock
        self.c.execute(
            """INSERT INTO stock(store_id, product_id, qty)
        VALUES (?,?,?) ON CONFLICT(store_id, product_id) DO UPDATE SET qty = qty + ?""",
            (store_id, product_id, qty, qty),
        )
        # mark item as received
        self.c.execute(
            """UPDATE restock_order_details SET received = 1 WHERE restock_order_id = ? AND product_id = ?""",
    ...
    def update_restock_status(self, restock_id):
        ...
        self.c.execute(
            """SELECT received FROM restock_order_details WHERE restock_order_id = ?""",
            (restock_id,),
        )
        received = self.c.fetchall()
        if all(received):
            self.c.execute(
                """UPDATE restock_orders SET status_id = 2 WHERE restock_order_id = ?""",
                (restock_id,),
            )
            self.conn.commit()
        # if some items are received, update restock status to restock_id=1
            ...
            self.c.execute(
                """UPDATE restock_orders SET status_id = 1 WHERE restock_order_id = ?""",
                (restock_id,),
            )
            self.conn.commit()
        ...
        # if no items are received, update restock status to restock_id=0
            ...
            self.c.execute(
                """UPDATE restocks SET status_id = 0 WHERE restock_order_id = ?""",
                (restock_id,),
            )
            self.conn.commit()
    ...
    def show_catalog(self, store_id):
        self.c.execute(
            """SELECT products.product_id, products.product_name, stock.qty, products.category_id 
        FROM products LEFT JOIN stock ON stock.product_id = products.product_id WHERE store_id = ?""",
            (store_id,),
        )
    ...
    def show_store_inventory(self, store_id):
        self.c.execute(
            """SELECT product_id, qty FROM stock WHERE store_id = ?""", (store_id,)
        )
    ...
    def fill_example_db(
        self,
        stores_csv="examples/stores.csv",
        products_csv="examples/products.csv",
        categories_csv="examples/categories.csv",
        statuses_csv="examples/statuses.csv",
    ):
